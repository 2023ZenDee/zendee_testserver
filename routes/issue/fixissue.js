...
const fixedIssue = async(req,res) => {
    ...
    if(req.user.userIdx !== findIssue.authorIdx){
        return res.status(200).send(
            authUtil.successTrue(statusCode.UNAUTHORIZED, responseMessage.ISSUE_ONLY_AUTHOR)
        )
    }
    ...
    return res.status(200).send(
        authUtil.successTrue(statusCode.OK, responseMessage.ISSUE_FIX_SUCCESS,updatedIssue)
    )
    }catch(err){
        console.log(err);
        return res.status(200).send(
            authUtil.successFalse(statusCode.INTERNAL_SERVER_ERROR, responseMessage.ISSUE_FIX_SERVER_ERROR)
        )
    } 
}
