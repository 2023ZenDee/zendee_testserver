module.exports = {
  REPORTED_ISSUE_SUCCESS: "신고된 이슈를 불러왔습니다.",
  REPORTED_ISSUE_FALSE: "이슈를 불러오지 못했습니다 :(",

  REPORTED_USER_SUCCESS: "신고된 유저를 불러왔습니다.",
  REPORTED_USER_FALSE: "유저를 불러오지 못했습니다 :(",

  REGION_BY_ISSUE_ERROR: "지역별 게시물 수를 불러오는데 문제가 발생하였습니다.",
  REGION_BY_ISSUE_SUCCESS: "지역별 게시물 조회 성공",

  TAG_BY_ISSUE_SUCCESS: "태그별 게시물 조회 성공",
  TAG_BY_ISSUE_ERROR : "태그별 게시물 조회 실패",
  
  NOT_FOUND_USER : "유저를 찾을 수 없습니다.",
  USER_FOUND_SUCCESS : "유저를 성공적으로 조회했습니다.",

  SERVER_ERROR : "서버 에러"

};